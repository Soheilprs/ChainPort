...
export async function POST(
  ...
  const fallback = `/app/compatibility/${id}`;
  ...
  try {
    apiResponse = await fetch(`${API_URL}/v1/compatibility-runs/${id}/migration-plans`, {
      method: "POST",
      headers: {
  ...
  return NextResponse.redirect(new URL(`/app/migrations/${planId}`, request.url), 303);
}
