...
docker build -t chainport/rpc-proxy:1 -f "$root/infrastructure/docker/rpc-proxy/Dockerfile" "$root"
echo "sandbox images built"
docker image inspect --format '{{.RepoTags}} {{.Id}}' chainport/sandbox-foundry:1 chainport/sandbox-node22:1 chainport/sandbox-node20:1 chainport/rpc-proxy:1
